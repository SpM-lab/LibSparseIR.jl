...
@testitem "MatsubaraSampling" begin
    ...
    function create_matsubara_sampling(basis::Ptr{LibSparseIR.spir_basis}, positive_only::Bool)
        ...
        points_status = LibSparseIR.spir_basis_get_n_default_matsus(basis, positive_only, n_matsubara_points_ref)
        @test points_status == LibSparseIR.SPIR_COMPUTATION_SUCCESS
        n_matsubara_points = n_matsubara_points_ref[]

        smpl_points = Vector{Int64}(undef, n_matsubara_points)
        get_points_status = LibSparseIR.spir_basis_get_default_matsus(basis, positive_only, smpl_points)
        @test get_points_status == LibSparseIR.SPIR_COMPUTATION_SUCCESS
    ...
    function test_matsubara_sampling_constructor(statistics::Integer)
        ...
        status = LibSparseIR.spir_basis_get_n_default_matsus(basis, false, n_points_org_ref)
        @test status == LibSparseIR.SPIR_COMPUTATION_SUCCESS
        n_points_org = n_points_org_ref[]
        @test n_points_org > 0

        smpl_points_org = Vector{Int64}(undef, n_points_org)
        points_status = LibSparseIR.spir_basis_get_default_matsus(basis, false, smpl_points_org)
        @test points_status == LibSparseIR.SPIR_COMPUTATION_SUCCESS
        ...
        positive_status = LibSparseIR.spir_basis_get_n_default_matsus(basis, true, n_points_positive_only_org_ref)
        ...
        positive_points_status = LibSparseIR.spir_basis_get_default_matsus(basis, true, smpl_points_positive_only_org)
