@testitem "freq.jl" begin
    using LibSparseIR
    @testset "freq" begin
        @test LibSparseIR.zeta(MatsubaraFreq(2)) == 0
        @test LibSparseIR.zeta(MatsubaraFreq(-5)) == 1
        ...
        @test LibSparseIR.value(pioverbeta, 3) == π / 3
        @test LibSparseIR.valueim(2 * pioverbeta, 3) == 2im * π / 3
    ...
    @testset "unit tests" begin
        @test_throws DomainError LibSparseIR.Statistics(2)
        @test LibSparseIR.Statistics(0) + LibSparseIR.Statistics(1) == Fermionic()
