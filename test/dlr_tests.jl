@testitem "DLR Constructor" tags=[:julia] begin
    using LibSparseIR
    using Test
    using Random
    using LinearAlgebra
    ...
    @testset "Constructor with default poles" begin
        ...
        @test dlr isa LibSparseIR.AbstractBasis
        @test LibSparseIR.statistics(dlr) isa Fermionic
        @test LibSparseIR.β(dlr) == β
        @test LibSparseIR.ωmax(dlr) == ωmax
        @test LibSparseIR.Λ(dlr) == β * ωmax
        @test LibSparseIR.accuracy(dlr) ≤ ε
        @test length(dlr) == length(dlr.poles)
        @test size(dlr) == (length(dlr),)
        @test npoles(dlr) == length(dlr.poles)
        @test get_poles(dlr) ≈ dlr.poles
        @test all(isone, LibSparseIR.significance(dlr))
        @test !LibSparseIR.iswellconditioned(dlr)
    ...
    @testset "Constructor with custom poles" begin
        ...
        @test LibSparseIR.statistics(dlr_custom) isa Bosonic
...
@testitem "IR to DLR transformation" tags=[:julia] begin
    using LibSparseIR
    using Test
    using Random
    using LinearAlgebra
...
@testitem "DLR with sampling" tags=[:julia] begin
    using LibSparseIR
    using Test
    using Random
    using LinearAlgebra
...
@testitem "DLR properties" tags=[:julia] begin
    using LibSparseIR
    using Test
    ...
    @testset "Basis properties inheritance" begin
        for stat in (Fermionic(), Bosonic())
            ...
            @test LibSparseIR.statistics(dlr) == stat
            @test LibSparseIR.β(dlr) == β
            @test LibSparseIR.ωmax(dlr) == ωmax
            @test LibSparseIR.Λ(dlr) == β * ωmax
            @test LibSparseIR.accuracy(dlr) ≤ ε
            ...
            @test all(isone, LibSparseIR.significance(dlr))
            @test !LibSparseIR.iswellconditioned(dlr)
            @test LibSparseIR.basis(dlr) === basis
        end
    end
